...
Engine::Engine(Display *display, Camera *camera)
{
	...
	m_planes.push_back(Plane(vec3(0, 0, -7), vec3(0, 0, 1), mat_w_diffuse));
	m_planes.push_back(Plane(vec3(-4, 0, 0), vec3(1, 0, 0), mat_b_reflective));
	m_planes.push_back(Plane(vec3(4, 0, 0), vec3(-1, 0, 0), mat_w_diffuse));
}
...
void Engine::render()
{
	...
	for (int y = 0; y < h; y++)
	{
		for (int x = 0; x < w; x++)
		{
			...
			// Raytrace
			auto &radiance = raytrace(r_primary, 0);
