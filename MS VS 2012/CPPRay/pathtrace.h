vec3 Engine::pathtrace(const Ray &r, int n, float weight, unsigned short *Xi)
{
    ...
    {
        //m_xLights.push_back(xFinal); <-- needs work
        //m_xLights.resize(MAXLIGHTPOSITIONS);
	...
	if (M_info.getReflT() == DIFF)
	{
        ...
        vec3 L_rand = vec3::sampleHemisphere(N_vector, Xi);
		...
		auto BRDF = f * 2.0f * NdotL * PI_1;

		// Calculate the contributed reflected light amount
		auto REFL = pathtrace(Ray(P_vector, L_rand), n + 1, weight, Xi);
